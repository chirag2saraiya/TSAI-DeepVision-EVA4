...
S3_BUCKET = os.environ['S3_BUCKET'] if 'S3_BUCKET' in os.environ else 'gauravp-face-recognition-model'
MODEL_PATH = os.environ['MODEL_PATH'] if 'MODEL_PATH' in os.environ else 'face_recognition_model.pt'
...
def recognize_image(event, context):
    try:
	    ...
	    #print(prediction)
        className = class_idx[prediction]
